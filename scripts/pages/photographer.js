...
async function init() {
  ...
  const mediaArray = data.media.filter(
  ...
  document
    .querySelector("main")
    .insertAdjacentHTML("beforeend", photographer.createPageHTML());
  ...
  mediaArray.forEach((mediaData) => {
    ...
    sectionPhotos.insertAdjacentHTML(
      "beforeend",
  ...
  // Fonction pour afficher les médias selon le tableau fourni
  function displayMedias(array) {
    const sectionPhotos = document.querySelector(".sectionphotos");
    sectionPhotos.innerHTML = ""; // vide la section avant de réinjecter
  ...
  sortSelect.addEventListener("change", (e) => {
    ...
    displayMedias(sortedArray);
    // Réactive les clics sur les médias après le tri
